<template>
  <div id="app">
    ...
    <template v-else>
      <component  is="UnitCalc" ref="node" @callbackReturn="callbackReturn"></component>
    </template>
  </div>
</template>
...
for(let key of Object.keys(modules)){
  ...
  if (module.info instanceof Function){
    let tempItems = module.info();
    if (tempItems instanceof Array){
      for(let tempItem of tempItems){
        items.push({...tempItem})
      }
    }else{
      items.push({...module.info})
    }
  }else{
    items.push({...module.info})
  }
